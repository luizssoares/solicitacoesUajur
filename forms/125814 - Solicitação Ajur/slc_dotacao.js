function verificaPAR() {
    var arrayRecurso = document.getElementById("arrUnidade").value
    var checkSolicitação = document.getElementById("novaSolicitacao").checked
    console.log(arrayRecurso)
    console.log("checkSolicitação: "+checkSolicitação)

    if (arrayRecurso != '' || checkSolicitação == true) {
        console.log("é um subprocesso")
        var linha = arrayRecurso.split("_").length
        ...
        for (var i = 1; i < linha; i++) {
            ...
            console.log("entrou array ... linha: " + i)
            slcProjeto(i)
            ...
            // var arrcodigo = (arrayRecurso.split("_")[i-1])
            var fonte = document.getElementById("arrUnidade").value.split("_")[i-1].split("/")[0]
            console.log(fonte)
            var valorU = document.getElementById("arrUnidade").value.split("_")[i-1].split("/")[1]
            console.log(valorU)
            // console.log(arrcodigo[i-1].split("/")[2])
            var codigo = document.getElementById("arrUnidade").value.split("_")[i-1].split("/")[2].split(".")
            console.log(codigo)

            console.log(codigo)
            for (var j = 0; j < Array.length; j++) {
                if (document.getElementById("arrUnidade").value.split('_')[i-1] != "") {
                    var acao = codigo[0] + '.' + codigo[1]
                    var unidade = codigo[0] + '.' + codigo[1] + '.' + codigo[2]
                    // console.log(acao)

                    if (codigo[0] == Array[j].CODIGO && (Array[j].CODIGO).split('.')[1] == null) {
                        console.log("PROJETO")
                        console.log("CODIGO: "+Array[j].CODIGO)
                        console.log("NOME: "+Array[j].NOME)
                        console.log(document.getElementById("slc_projeto_" + i))
                        document.getElementById("slc_projeto_" + i).value = Array[j].NOME
                        document.getElementById("txt_codprojeto___" + i).value = Array[j].CODIGO
                    }
                        ...
                        console.log("CODIGO: "+Array[j].CODIGO)
                        console.log("NOME: "+Array[j].NOME)
                        console.log(document.getElementById("slc_acao_" + i))
                        document.getElementById("slc_acao_" + i).value = Array[j].NOME
                        document.getElementById("txt_codacao___" + i).value = Array[j].CODIGO
                    }

                    else if (unidade == (Array[j].CODIGO)) {
                        console.log("UNIDADE")
                        console.log("CODIGO: "+Array[j].CODIGO)
                        console.log("NOME: "+Array[j].NOME)
                        console.log(document.getElementById("slc_unidade_" + i))
                        document.getElementById("slc_unidade_" + i).value = Array[j].NOME
                        document.getElementById("txt_codrecurso___" + i).value = Array[j].CODIGO
                    }

                    document.getElementById("slc_FonteRecursos___" + i).value = fonte
                    document.getElementById("txt_valorUtil___" + i).value = valorU
                }
            }
            console.log("------------------------------------------------------------------------")
        }
        ...
        console.log('não é um subprocesso')
    }
    document.getElementById("novaSolicitacao").checked = false    
}
...
function slcProjeto(linha) {
    ...
    else if (Now == 5 || Now == 55 || Now == 181 ||  Now == 107 || Now == 13 || Now == 51 || Now == 54 || Now == 14 || Now == 15 || Now == 16 || Now == 18 || Now == 98) {
        att = document.createAttribute('readonly')
        vselect.setAttributeNode(att)
    }
    ...
    document.getElementById("slc_unidade_" + linha).addEventListener('change', function () {
        ...
        for (i = 0; i < arrayOption.length; i++) {
            ...
            if (arrayOption[i].NOME == codUni && Proj == codPro) {
                ...
                // document.getElementById("arrUnidade").value += document.getElementById("txt_codrecurso___" + linha).value + "_"
                break;
            }
        }
        console.log(document.getElementById("arrUnidade").value)
    })
}
...
function loadSubProcess() {
    ...
    if (Now == 4) {
        verificaPAR()
        secFunc(document.getElementById('tp_solicitacao').value)
    }

    document.getElementById("novaSolicitacao").addEventListener('click',function(){
        document.getElementById('arrUnidade').value = ''
        var linha = getRowDot()
        console.log(linha)
        for(var i=1; i<=linha; i++){
            console.log(document.getElementById("txt_codrecurso___"+i))
            // var arrayReal = []
            document.getElementById('arrUnidade').value += document.getElementById('slc_FonteRecursos___'+i).value + '/'
            document.getElementById('arrUnidade').value += document.getElementById('txt_valorUtil___'+i).value + '/'
            document.getElementById('arrUnidade').value += document.getElementById("txt_codrecurso___"+i).value + "_"   
        }  
        console.log(document.getElementById('arrUnidade').value)
    });

}
window.addEventListener('load', loadSubProcess)